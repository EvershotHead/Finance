...
from src.analysis.return_distribution import analyze_return_distribution
from src.analysis.risk_metrics import analyze_risk
from src.analysis.benchmark_comparison import analyze_benchmark
from src.analysis.ols_capm import analyze_ols_capm
from src.analysis.factor_models import analyze_factor_model
from src.analysis.time_series_tests import analyze_time_series
from src.analysis.arma_model import analyze_arma
from src.analysis.volatility_models import analyze_garch
...
from src.analysis.rolling_analysis import analyze_rolling
from src.analysis.backtesting import backtest_dual_ma, backtest_rsi, backtest_bollinger
from src.analysis.scoring import calculate_score
from src.report.interpretation import generate_conclusion_summary
from src.report.generator import generate_json_report, generate_markdown_report, generate_html_report, generate_csv_export
...
from src.utils.date_utils import get_default_start_date, get_default_end_date
...
def _get_result(results: dict, key: str, field: str, default=None):
    ...
    Args:
        results: 分析结果字典
        key: 模块键名
        field: 属性名 (success/data/interpretation 等)
        default: 默认值
    """
    obj = results.get(key)
    if obj is None:
...
def render_sidebar():
    ...
    stock_name = st.sidebar.text_input("股票名称", value="延江股份", help="仅用于报告显示")
    stock_code = st.sidebar.text_input("股票代码", value="300658", help="如 300658 或 300658.SZ")
...
def run_analysis(params: dict):
    ...
    try:
        ...
        st.session_state["stock_data"] = stock_bundle
        ...
        if params["run_backtest"]:
            ...
            if bt_results:
                results["backtesting"] = bt_results[0]  # 取第一个作为代表
        
        # 综合评分 - 使用 getattr 处理 dataclass 对象
...
def _render_data_table(key, results, columns=["指标", "值"]):
    """通用：渲染数据表格（确保值列为字符串以避免 Arrow 序列化错误）"""
